...
func TestRoadmapSchemasMatchRuntimeRequiredFields(t *testing.T) {
	...
	checks := map[string][]string{
		"harness/schemas/study-manifest.schema.json":            {"repository", "revision", "case_corpus_digest", "oracle_corpus_digest", "model_config_digest"},
	...
	for path, required := range checks {
		...
		for _, v := range required {
			if !seen[v] {
				t.Errorf("%s missing required %s", path, v)
			}
		}
	}
	body, err := os.ReadFile(filepath.Join(root, "harness/schemas/policy-pack.schema.json"))
	if err != nil {
		t.Fatal(err)
	}
	if strings.Contains(string(body), `"exceptions"`) {
		t.Fatal("policy pack schema still permits unsigned exceptions")
	}
}
func TestRoadmapFixturesParseRuntimeShapes(t *testing.T) {
	root := filepath.Join("..")
	var result map[string]any
	body, err := os.ReadFile(filepath.Join(root, "harness/studies/valid-study-result.json"))
	if err != nil {
		t.Fatal(err)
	}
	if err := json.Unmarshal(body, &result); err != nil || result["schema_version"] != "1.0.0" || len(result["manifest_digest"].(string)) != 64 {
		t.Fatalf("invalid study fixture: %v", err)
	}
	var approval map[string]any
	body, err = os.ReadFile(filepath.Join(root, "harness/policies/valid-policy-exception-approval.json"))
	if err != nil {
		t.Fatal(err)
	}
	if err := json.Unmarshal(body, &approval); err != nil || approval["scope"] != "required" || approval["approver"] == "" {
		t.Fatalf("invalid approval fixture: %v", err)
	}
}
