...
func ParseAll(values []string) ([]int, error) {
	...
	for i, raw := range values { v, err := strconv.Atoi(strings.TrimSpace(raw)); if err != nil { failures = append(failures, fmt.Errorf("item %d: %w", i, err)); continue }; out = append(out, v) }
