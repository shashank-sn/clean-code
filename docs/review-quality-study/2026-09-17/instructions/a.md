...
Required input: final diff, requirements, and revision-bound evidence

Required output: evidence-backed findings or correct silence

Evidence: Preserve revision-bound evidence and report unavailable work honestly.

Stop conditions: Stop when required evidence is missing, stale, or contradicts completion.; Stop when the change author and the approver share the same context.
...
Available: read_repository, write_repository
