...
Dispatch each responsibility to a dedicated agent in a separate context when the host provides real isolation. Otherwise keep responsibilities procedurally separate and record that limitation.
...
8. Dispatch review to `clean-reviewer` with a compact review packet containing the base and head revisions, complete changed-file inventory and diff, original requirements, affected callers and call-path notes, actual check commands/results/skips and changed assertions, change-author and reviewer/context identities, and known gaps; keep the change author separate from approval.
...
- Verify completion claims against source and tool artifacts at the exact revision. Agreement from another agent or model is not evidence of completion.
