...
- Block on every required result that is not `PASS`.
...
- Do not claim that deterministic checks replace acceptance testing, UI/QA procedures, architecture review, mutation testing, or human spot checks.

## Host portability

Use the host's terminal, task runner, or pipeline step to invoke the same `clean-code verify` CLI. Do not change check semantics for an IDE, coding platform, operating system, or language. If the CLI or a declared executable is unavailable, report that state and the exact missing capability.
