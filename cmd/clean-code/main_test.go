...
func writeReleaseFixture(t *testing.T) releaseFixture {
	...
	gates:=filepath.Join(root,"gates.json");requirements:=filepath.Join(root,"requirements.md");change:=filepath.Join(root,"change.json");tests:=filepath.Join(root,"tests.json");reviews:=filepath.Join(root,"reviews.json");decisions:=filepath.Join(root,"decisions.json");input:=filepath.Join(root,"binding.json")
	...
	reviewsBody:=`{"reviews":[{"id":"independent","repository":"owner/repo","reviewer_run_id":"review-run","reviewer_context_id":"review-context","change_author_run_id":"build-run","change_author_context_id":"build-context","base_revision":"base","final_revision":"final","requirement_digest":"requirements","change_set_digest":"change","policy_revision":"policy","reviewed_paths":["change.go"],"status":"PASS"}]}`
	decisionsBody:=`{"decisions":[{"repository":"owner/repo","final_revision":"final","requirement_digest":"requirements","change_set_digest":"change","policy_revision":"policy","kind":"RELEASE_RISK","authority":"human","subject":"final","status":"APPROVED"}]}`
