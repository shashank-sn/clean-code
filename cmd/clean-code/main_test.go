...
func writeReleaseFixture(t *testing.T) releaseFixture {
	...
	gatesBody:=`{"policy_revision":"policy","required_tests":["acceptance"],"required_reviews":["independent"],"required_decisions":["RELEASE_RISK"]}`;requirementsBody:="R1: release safely\n";changeBody:=`{"schema_version":"1.0.0","base_revision":"base","final_revision":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","changed_paths":["change.go"]}`
	reqD,changeD,policyD:=digest(requirementsBody),digest(changeBody),digest(gatesBody)
	testsBody:=`{"tests":[{"id":"acceptance","repository":"owner/repo","run_id":"test-run","requirement_ids":["R1"],"revision":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","status":"PASS","artifact_digest":"artifact","actor_run_id":"test-actor","started_at":"2026-08-12T00:00:00Z","finished_at":"2026-08-12T00:00:00Z"}]}`
	reviewsBody:=fmt.Sprintf(`{"reviews":[{"id":"independent","repository":"owner/repo","reviewer_run_id":"review-run","reviewer_context_id":"review-context","change_author_run_id":"build-run","change_author_context_id":"build-context","base_revision":"base","final_revision":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","requirement_digest":"%s","change_set_digest":"%s","policy_revision":"%s","reviewed_paths":["change.go"],"status":"PASS"}]}`,reqD,changeD,policyD)
	decisionsBody:=fmt.Sprintf(`{"decisions":[{"repository":"owner/repo","final_revision":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","requirement_digest":"%s","change_set_digest":"%s","policy_revision":"%s","kind":"RELEASE_RISK","authority":"human","subject":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","status":"APPROVED"}]}`,reqD,changeD,policyD)
	...
	binding:=fmt.Sprintf(`{"schema_version":"1.0.0","repository":"owner/repo","base_revision":"base","final_revision":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","requirement_digest":"%s","change_set_digest":"%s","policy_revision":"%s","test_attestations_digest":"%s","review_attestations_digest":"%s","decision_attestations_digest":"%s","changed_paths":["change.go"]}`,reqD,changeD,policyD,digest(testsBody),digest(reviewsBody),digest(decisionsBody));write(input,binding)
	manifest:=fmt.Sprintf(`{"schema_version":"1.0.0","repository":"owner/repo","final_revision":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","binding_digest":"%s","policy_digest":"%s","requirements_digest":"%s","change_set_digest":"%s","test_attestations_digest":"%s","review_attestations_digest":"%s","decision_attestations_digest":"%s"}`,digest(binding),policyD,reqD,changeD,digest(testsBody),digest(reviewsBody),digest(decisionsBody));write(approvalPath,manifest)
...
func TestReleaseGateRejectsModifiedChangeSetArtifact(t *testing.T){f:=writeReleaseFixture(t);if err:=os.WriteFile(f.change,[]byte(`{"schema_version":"1.0.0","base_revision":"base","final_revision":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa","changed_paths":["other.go"]}`),0o600);err!=nil{t.Fatal(err)};code,msg:=runReleaseFixture(f);if code!=1||!strings.Contains(msg,"trusted artifact digest"){t.Fatalf("got %d %s",code,msg)}}
...
func TestReleaseGateRejectsConsistentForgeryWithOldSignature(t *testing.T){f:=writeReleaseFixture(t);body,_:=os.ReadFile(f.approval);body=bytes.Replace(body,[]byte(`"final_revision":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"`),[]byte(`"final_revision":"bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb"`),1);if err:=os.WriteFile(f.approval,body,0o600);err!=nil{t.Fatal(err)};code,msg:=runReleaseFixture(f);if code!=1||!strings.Contains(msg,"signature verification failed"){t.Fatalf("got %d %s",code,msg)}}
