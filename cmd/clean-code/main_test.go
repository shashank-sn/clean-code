...
func TestReleaseGateRejectsConsistentForgeryWithOldSignature(t *testing.T){f:=writeReleaseFixture(t);body,_:=os.ReadFile(f.approval);body=bytes.Replace(body,[]byte(`"final_revision":"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"`),[]byte(`"final_revision":"forged"`),1);if err:=os.WriteFile(f.approval,body,0o600);err!=nil{t.Fatal(err)};code,msg:=runReleaseFixture(f);if code!=1||!strings.Contains(msg,"signature verification failed"){t.Fatalf("got %d %s",code,msg)}}
