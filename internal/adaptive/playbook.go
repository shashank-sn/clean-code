...
var builtinPlaybooks = map[string]Playbook{
	...
	"code-review": {
		...
		MinimumEvidence:  []string{"diff", "requirements", "verification-report", "review-packet", "requirement-call-path-test-matrix"},
		ArenaWarranted:   "no",
		AdversarialProbesWarranted: "review may request probes; it does not replace them",
		ParallelGuidance: "use separate reviewer contexts only when the host provides real isolation; otherwise run standards and spec passes sequentially in one context and record procedural separation plus the limitation; reconcile blocking dispositions before ship",
		TerminalStates:             []string{TerminalPass, TerminalFail, TerminalNotRun, TerminalNotAvailable, TerminalBlocked},
	},
	"release-readiness": {
		SchemaVersion:    SchemaVersion,
