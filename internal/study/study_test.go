...
func fixture() (Manifest, Results) {
	m := Manifest{SchemaVersion: "1.0.0", StudyID: "s", Repository: "o/r", Revision: rev, CaseCorpusDigest: digest64, OracleCorpusDigest: digest64, ModelConfigDigest: digest64, MinimumPairs: 1, Tasks: []Task{{ID: "t", Model: "m", Tools: []string{"x"}, Limit: 1, Oracle: "o"}}}
	out := func(arm, run, artifact string) Outcome {
		return Outcome{Repository: "o/r", Revision: rev, RunID: run, ArtifactDigest: Digest([]byte(artifact)), StartedAt: "2026-08-12T00:00:00Z", FinishedAt: "2026-08-12T00:01:00Z", TaskID: "t", Arm: arm, Model: "m", Tools: []string{"x"}, Limit: 1, Oracle: "o", CaseCorpusDigest: digest64, OracleCorpusDigest: digest64, ModelConfigDigest: digest64, Status: "PASS"}
	}
...
func TestVerifyInputsRejectsCorpusAndConfigReplay(t *testing.T) {
	...
	if err := VerifyInputs(m, cases, oracle, config); err != nil {
		t.Fatal(err)
	}
	if err := VerifyInputs(m, []byte("changed"), oracle, config); err == nil {
		t.Fatal("expected changed case corpus rejection")
	}
	if err := VerifyInputs(m, cases, []byte("changed"), config); err == nil {
		t.Fatal("expected changed oracle corpus rejection")
	}
	if err := VerifyInputs(m, cases, oracle, []byte("changed")); err == nil {
